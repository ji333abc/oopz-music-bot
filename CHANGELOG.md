...
- `oopzctl upgrade` 实时显示阶段进度条、当前操作和累计耗时，并流式输出 Git/Docker 日志；失败时显示回滚状态。进度写入 stderr，stdout 保留最终 JSON，管道日志使用普通行输出。
- Panel 播放控制新增 0～100% 音量条，调整后保存到 Redis，语音重连和 Bot 重启后恢复。
...
- QQ 音乐搜索切换到 `musicu` 桌面搜索接口，解决旧 `client_search_cp` 返回 HTTP 500 导致热门歌曲搜不到；保留原有候选结果格式和分页。Docker 默认旧版播放器现在保留搜索上游错误，不再请求不适用的 `/search` 或将故障误报为未找到。
- 升级预检查按 `data/` 大小估算暂存副本和压缩备份所需空间，并在备份完成后再次检查余量；空间不足时显示估算值和清理位置，避免只满足 1 GiB 就进入大数据备份。
- Docker 默认语音播放器改用 URL 边拉取边推流，取消每首歌并行备用下载和下一首音频预下载；直推失败不再导致后续五分钟全部转为下载。备用播放只使用内存（单首上限 64 MiB），不再创建音频临时文件，并在停止或播放结束时释放媒体资源。
- 搜歌时保留接口超时、连接失败等真实错误，避免误报“QQ音乐未找到”；失败历史区分上游故障与空结果，不再统一显示 `unknown`。搜索故障不会进入负缓存。
- Panel 修复六处静态类型错误：兼容字符串请求 ID，校验健康状态、接口指标及缓存数据，无效更新时间显示“时间未知”，避免队列规范化时把数组下标当作条目标识。
- 未保存音量时，实际推流与页面默认音量统一为 30%，避免初次播放使用 100% 音量。
...
- 搜索接口修复需要重建 QQMusic 和 Bot 服务；无新增配置或凭证迁移，继续使用现有 Cookie 及自动续期。回滚可切回旧提交并重建这两个服务，但旧搜索接口可能仍返回 500。
- URL 流式推流修复需要重建并重启 Bot，现有暂停、进度和音量控制继续可用；无需新配置或数据迁移，旧版遗留临时文件不会自动删除。回滚可切回旧提交并重建 Bot。
- 默认音量可通过 `OOPZ_DEFAULT_VOLUME` 覆盖（默认 30）；已有 Redis 音量优先，更新需重建 Bot 和 Panel。回滚可切回旧提交并重建这两个服务，已保存的音量仍保留。
