...
class VoiceClient:
    ...
    def _browser_thread_loop(self):
        ...
        try:
            ...
            logger.warning("Playwright 不可用 (%s)，尝试 Selenium 后端...", err_msg[:160])
    ...
    def _run_playwright_backend(self):
        ...
        async def _init():
            ...
            executable = str(
                os.getenv("BOT_CHROMIUM_EXECUTABLE_PATH")
                or os.getenv("OOPZ_VOICE_BROWSER_EXECUTABLE_PATH")
    ...
    def _run_selenium_backend(self):
        ...
        # 方式 1：优先使用镜像内与 Chromium 同版本的系统 ChromeDriver。
        driver_path = str(
            os.getenv("BOT_CHROMEDRIVER_PATH")
            or ("/usr/bin/chromedriver" if os.path.isfile("/usr/bin/chromedriver") else "")
        ).strip()
        if driver_path and os.path.isfile(driver_path):
            try:
                ...
                last_error = e
                logger.debug("系统 ChromeDriver 方式失败: %s", e)

        # 方式 2：webdriver-manager 自动下载 ChromeDriver
        if driver is None:
            try:
        ...
        # 方式 3：Selenium 4.6+ 自带的 Selenium Manager（不传 service）
        if driver is None:
            try:
        ...
        # 方式 4：指定 Windows 常见 Chrome 路径，再试 Selenium Manager
        ...
        # 方式 5（仅 Windows）：尝试 Edge 浏览器（系统常预装）
        if driver is None and os.name == "nt":
            try:
    ...
    def play_audio(self, url: str, on_started=None, fallback_url: Optional[str] = None):
        """优先由浏览器直读主链接，Python 仅下载低码率备用链接。"""
        if not self._available:
            return
        self.stop_audio()
        ...
        with self._preload_lock:
            cached = self._preloaded.pop(fallback_url, None)
        audio_data, content_type = cached if cached else (None, "audio/mpeg")
    ...
    def stop_audio(self):
        ...
        self._fallback_download_stop.set()
