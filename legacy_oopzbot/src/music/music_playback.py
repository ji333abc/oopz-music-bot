...
class PlaybackMixin:
                    ...
                    with self._playback_lock:
                        ...
                        if (queue_length > 0 or finished_song is not None) and current is None:
                            ...
                            if next_song:
                                ...
                                play_queue = (
                                    self._get_queue(ar)
                                    if ar and hasattr(self, "_get_queue")
                                    else self.queue
                                )
                                play_queue.set_current(next_song)
    ...
    def _resolve_fallback_audio_url(self, song_data: dict, default_url: str) -> str:
        ...
        platform_name = song_data.get("platform", "netease")
        song_id = song_data.get("song_id")
        platform = self.platforms.get(platform_name) if hasattr(self, "platforms") else None
        getter = getattr(platform, "get_fallback_song_url", None)
        if callable(getter) and song_id:
            try:
                return getter(song_id) or default_url
            except Exception as e:
                logger.debug(f"获取备用音频 URL 失败，沿用主链接: {e}")
        return default_url

    def _preload_next_song_if_any(self):
        """预加载下一首的低码率备用音频，主链接仍由 Chromium 直读。"""
        if not self.voice or not self.voice.available:
            return
        try:
            next_item = self.queue.peek_next()
            if next_item and next_item.get("url"):
                fallback_url = self._resolve_fallback_audio_url(
                    next_item,
                    next_item["url"],
                )
                self.voice.preload_audio(fallback_url)
        except Exception as e:
    ...
    def _stream_to_voice_channel(self, url: str, name: str, channel: str, area: str,
        ...
        try:
            ...
            fallback_url = self._resolve_fallback_audio_url(current, url)
            ...
            if song_id:
                ...
                try:
                    ...
                    if new_url:
                        retry_data = dict(current)
                        retry_data.setdefault("song_id", song_id)
                        new_fallback_url = self._resolve_fallback_audio_url(
                            retry_data,
                            new_url,
                        )
                        self.voice.play_audio(
    ...
    def _is_playing(self) -> bool:
        ...
        if self._play_start_time <= 0:
            return False
        try:
            active_area = getattr(self, "_voice_channel_area", None)
            q = (
                self._get_queue(active_area)
                if active_area and hasattr(self, "_get_queue")
                else self.queue
            )
            ps = q.get_play_state()
            if ps:
                # _start_playing() 会先写入 loading=true，随后 Chromium/Agora
                # 真正开始播放时才改为 loading=false。在此期间 voice.is_playing
                # 仍可能为 False，不能把刚提交的歌曲误判为已经播完。
                if ps.get("paused") or ps.get("loading"):
                    return True
        except Exception as e:
